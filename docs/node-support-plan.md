...
TODO list:
[ ] Add a "-node" flag to rnwinrt.exe that puts it into "node" mode.  (we will keep it named "rnwinrt.exe" for now).
[ ] Next to the "rnwinrt\rnwinrt\react" dir we should have a "rnwinrt\rnwinrt\node" dir, that will have node-specific template files.
Let's start with empty node_base.h and node_base.cpp files for now.
[ ] In "samples", we want a NodeTestApp, that is a simple nodejs app.
[ ] NodeTestApp uses node-gyp to build a .node file.
[ ] NodeTestApp runs rnwinrt.exe (with the -node flag of course) to build projections for the TestComponent in this repo.
