...
{
    std::shared_ptr<IProjectedValueProvider> ProjectedNamespace::Create(const std::string_view& name, const std::shared_ptr<ProjectionsContext>& context, InitializerFunction initializer)
    {
        return std::make_shared<ProjectedHostObjectProvider>(name, std::make_shared<ProjectedNamespace>(name, context, initializer));
    }

    ProjectedNamespace::ProjectedNamespace(const std::string_view& name, const std::shared_ptr<ProjectionsContext>& context, InitializerFunction initializer)
        : m_name(name), m_context(context), m_initializer(initializer)
    {
    }
    ...
    {
        throw jsi::JSError(runtime, "TypeError: Cannot assign to property '" + propName.utf8(runtime) + "' to a projected WinRT namespace");
    }
