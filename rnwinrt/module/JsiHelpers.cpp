...
namespace WinRTTurboModule
{
    jsi::String CreateString(jsi::Runtime& runtime, const std::wstring_view& value)
    {
        ...
        // WideCharToMultiByte is requesting the size in utf8 characters required for 'value' without null termination as
        // std::string constructed with a fill will actually allocated 'outputLength + 1' characters and asign the last
        // as the null terminator automatically.

        const auto outputLength = ::WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, value.data(), static_cast<int32_t>(value.size()),
        ...
        winrt::check_bool(::WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, value.data(), static_cast<int32_t>(value.size()),
    ...
    jsi::Value PromiseWrapper::Create(jsi::Runtime& runtime, std::shared_ptr<PromiseWrapper>& wrapper)
    {
        ...
        jsi::Function callback = jsi::Function::createFromHostFunction(runtime, jsi::PropNameID::forAscii(runtime, "fn"), 2,
            [&wrapper](jsi::Runtime& runtime, const jsi::Value& thisVal, const jsi::Value* args, size_t count)
            {
                FAIL_FAST_IF(count < 2);
                wrapper = std::make_shared<PromiseWrapper>(args[0].asObject(runtime).asFunction(runtime), args[1].asObject(runtime).asFunction(runtime));
    ...
    void CallInvokerWrapper::CallSync(std::function<void()>&& func)
    {
        ...
        else
        {
            ...
            m_callInvokerWrapper->invokeAsync(
                [event, func{ std::forward<std::function<void()>>(func) }]()
                {
                    func();
                    event.SetEvent();
                });
