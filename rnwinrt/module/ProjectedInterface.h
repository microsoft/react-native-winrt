...
namespace WinRTTurboModule
{
    ...
    struct DECLSPEC_NOVTABLE IProjectedInterfaceInstance
    {
        ...
        virtual jsi::Value InvokeMethod(jsi::Runtime& runtime, const std::shared_ptr<ProjectedFunction>& method, const jsi::Value* args, const uint16_t numArgs) = 0;

        virtual jsi::Value GetProperty(jsi::Runtime& runtime, const std::shared_ptr<IProjectedPropertyBase>& property) = 0;
        virtual void SetProperty(jsi::Runtime& runtime, const std::shared_ptr<IProjectedPropertyBase>& property, const jsi::Value& value) = 0;

        virtual void AddEventListener(jsi::Runtime& runtime, const std::shared_ptr<IProjectedEventBase>& event, const jsi::Value& value) = 0;
        virtual void RemoveEventListener(jsi::Runtime& runtime, const std::shared_ptr<IProjectedEventBase>& event, const jsi::Value& value) = 0;
    };
    ...
    {
    public:
        static std::shared_ptr<IProjectedInterfaceInstance> Create(const std::shared_ptr<ProjectionsContext>& context, const winrt::Windows::Foundation::IInspectable& instance, std::shared_ptr<ProjectedInterface>&& projectedInterface)
        {
            // Use "new" instead of std::make_shared to avoid specializing std::shared_ptr<ProjectedInterfaceInstance<I>> to reduce the sizes of object files.
            return std::shared_ptr<ProjectedInterfaceInstance>(new ProjectedInterfaceInstance<I>(context, instance, std::forward<std::shared_ptr<ProjectedInterface>>(projectedInterface)));
        }

        ProjectedInterfaceInstance(const std::shared_ptr<ProjectionsContext>& context, const winrt::Windows::Foundation::IInspectable& instance, std::shared_ptr<ProjectedInterface>&& projectedInterface)
            : m_context(context), m_inspectableInstance(instance), m_projectedInterface(std::forward<std::shared_ptr<ProjectedInterface>>(projectedInterface))
        {
        }
        ...
        virtual jsi::Value InvokeMethod(jsi::Runtime& runtime, const std::shared_ptr<ProjectedFunction>& method, const jsi::Value* args, const uint16_t numArgs) override
        {
            EnsureInitialized();
        ...
        virtual jsi::Value GetProperty(jsi::Runtime& runtime, const std::shared_ptr<IProjectedPropertyBase>& property) override
        {
            EnsureInitialized();
            return static_cast<IProjectedProperty<I>*>(property.get())->Get(m_queriedInstance, m_context);
        }

        virtual void SetProperty(jsi::Runtime& runtime, const std::shared_ptr<IProjectedPropertyBase>& property, const jsi::Value& value) override
        {
            EnsureInitialized();
            static_cast<IProjectedProperty<I>*>(property.get())->Set(m_queriedInstance, m_context, value);
        }

        virtual void AddEventListener(jsi::Runtime& runtime, const std::shared_ptr<IProjectedEventBase>& event, const jsi::Value& value) override
        {
            EnsureInitialized();
            m_context->EventRegistrar.Get(m_inspectableInstance, event)->Add(m_queriedInstance, m_context, value);
        }

        virtual void RemoveEventListener(jsi::Runtime& runtime, const std::shared_ptr<IProjectedEventBase>& event, const jsi::Value& value) override
        {
            EnsureInitialized();
    ...
    // ProjectedInterface is not templated by the winrt interface to reduce the amount of specialized code. Avoiding specializing STL contains greatly
    // reduces the size of object files and static libs. For example, even std::enable_shared_from_this<ProjectedInterface<I>> is very expensive.
    ...
    public:
        ...
        using InstanceFactory = std::shared_ptr<IProjectedInterfaceInstance>(*)(const std::shared_ptr<ProjectionsContext>&, const winrt::Windows::Foundation::IInspectable&, std::shared_ptr<ProjectedInterface>&&);
        ...
        {
            return std::make_shared<ProjectedInterface>(winrt::guid_of<I>(), &ProjectedInterfaceInstance<I>::Create, initializer);
        }
        ...
        std::shared_ptr<IProjectedInterfaceInstance> CreateInstance(const std::shared_ptr<ProjectionsContext>& context, const winrt::Windows::Foundation::IInspectable& instance);
