...
napi_wrappers::String rnwinrt::make_string(napi_wrappers::Runtime& runtime, std::wstring_view str)
{
    if (str.empty())
    {
        return napi_wrappers::String::createFromAscii(runtime, "");
    }
    ...
    return napi_wrappers::String::createFromUtf8(runtime, reinterpret_cast<const uint8_t*>(buffer.get()), bytes);
}

std::u16string rnwinrt::string_to_utf16(napi_wrappers::Runtime& runtime, const napi_wrappers::String& string)
{
...
napi_wrappers::Value object_instance_cache::get_instance(napi_wrappers::Runtime& runtime, const winrt::IInspectable& value)
{
    ...
    if (auto itr = instances.find(key); itr != instances.end())
    {
        ...
        else
        {
            ...
            {
                // Re-create the wrapper from the cached HostObject using createFromHostObject
                // which will detect it's a projected_object_instance and create a WinRTObjectWrapper
                auto winrtObject = napi_wrappers::Object::createFromHostObject(runtime, hostObj);
                return napi_wrappers::Value(runtime, winrtObject);
            }
        }
    ...
    // Create a projected_object_instance (HostObject) to wrap the WinRT instance
    auto hostObj = std::make_shared<projected_object_instance>(instance);
    
    // Use Object::createFromHostObject which will detect it's a projected_object_instance
    // and create a WinRTObjectWrapper for better performance
    auto winrtObject = napi_wrappers::Object::createFromHostObject(runtime, hostObj);
    ...
    if (supports_weak_object)
    {
        try
        {
            instances.emplace(key, napi_wrappers::WeakObject(runtime, winrtObject));
        }
    ...
    if (!supports_weak_object)
    {
        // For the non-weak case, cache the HostObject so we can re-create the wrapper if needed
        instances.emplace(key, hostObj);
    }

    return napi_wrappers::Value(runtime, winrtObject);
}

napi_wrappers::Value static_namespace_data::create(napi_wrappers::Runtime& runtime) const
{
    return napi_wrappers::Value(runtime, napi_wrappers::Object::createFromHostObject(runtime, std::make_shared<projected_namespace>(this)));
}
...
napi_wrappers::Value static_enum_data::create(napi_wrappers::Runtime& runtime) const
{
    return napi_wrappers::Value(runtime, napi_wrappers::Object::createFromHostObject(runtime, std::make_shared<projected_enum>(this)));
}

napi_wrappers::Value static_enum_data::get_value(napi_wrappers::Runtime& runtime, std::string_view valueName) const
{
    ...
    if (itr != values.end())
    {
        return napi_wrappers::Value(Napi::Number::New(runtime.env(), itr->value));
    }
    ...
    if (itr != values.end())
    {
        return napi_wrappers::Value(runtime, make_string(runtime, itr->name));
    }

    return napi_wrappers::Value::undefined(runtime);
}

napi_wrappers::Value projected_enum::get(napi_wrappers::Runtime& runtime, const napi_wrappers::PropNameID& name)
{
...
napi_wrappers::Value static_class_data::create(napi_wrappers::Runtime& runtime) const
{
    return napi_wrappers::Value(
        runtime, napi_wrappers::Object::createFromHostObject(runtime, std::make_shared<projected_statics_class>(this)));
}

static napi_wrappers::Value static_add_event_listener(napi_wrappers::Runtime& runtime, const napi_wrappers::Value* args, size_t count,
    const static_class_data* data, event_registration_array& registrations)
{
    if (count < 2)
    {
        throw Napi::Error::New(runtime.env(), "TypeError: addEventListener expects (at least) 2 arguments");
    }
...
static napi_wrappers::Value static_remove_event_listener(napi_wrappers::Runtime& runtime, const napi_wrappers::Value* args, size_t count,
    const static_class_data* data, event_registration_array& registrations)
{
    if (count < 2)
    {
        throw Napi::Error::New(runtime.env(), "TypeError: removeEventListener expects (at least) 2 arguments");
    }
...
napi_wrappers::Value projected_statics_class::get(napi_wrappers::Runtime& runtime, const napi_wrappers::PropNameID& id)
{
...
void projected_statics_class::set(napi_wrappers::Runtime& runtime, const napi_wrappers::PropNameID& id, const napi_wrappers::Value& value)
{
    ...
    {
        throw Napi::Error::New(runtime.env(), "TypeError: Cannot assign to property '" + name + "' of a projected WinRT class");
    }
...
std::vector<napi_wrappers::PropNameID> projected_statics_class::getPropertyNames(napi_wrappers::Runtime& runtime)
{
    std::vector<napi_wrappers::PropNameID> result;
...
napi_wrappers::Value projected_statics_class::add_event_listener(napi_wrappers::Runtime& runtime, const napi_wrappers::Value* args, size_t count)
{
    return static_add_event_listener(runtime, args, count, m_data, m_events);
}

napi_wrappers::Value projected_statics_class::remove_event_listener(napi_wrappers::Runtime& runtime, const napi_wrappers::Value* args, size_t count)
{
    return static_remove_event_listener(runtime, args, count, m_data, m_events);
}

napi_wrappers::Value static_activatable_class_data::create(napi_wrappers::Runtime& runtime) const
{
    // Create the constructor function
    auto ctor = napi_wrappers::Function::createFromHostFunction(runtime,
        make_propid(runtime, name), 0, constructor);

    // Get the underlying Napi::Object so we can define properties
    auto obj = ctor.m_value.As<Napi::Object>();

    // Define getter/setter properties for each static property
    for (auto&& prop : properties)
    {
        auto propName = Napi::String::New(runtime.env(), prop.name.data(), prop.name.size());
        
        // Create getter lambda that captures prop by copy
        auto getter_fn = [prop_getter = prop.getter](const Napi::CallbackInfo& info) -> Napi::Value {
            napi_wrappers::Runtime rt(info.Env());
            auto value = prop_getter(rt);
            return value.m_value;
        };

        // Create setter lambda (if one exists)
        if (prop.setter)
        {
            auto setter_fn = [prop_setter = *prop.setter](const Napi::CallbackInfo& info) {
                if (info.Length() > 0)
                {
                    napi_wrappers::Runtime rt(info.Env());
                    napi_wrappers::Value val(rt, info[0]);
    ...
    {
        auto fn = napi_wrappers::Function::createFromHostFunction(runtime,
            make_propid(runtime, func.name), 0, func.function);
...
namespace rnwinrt
{
    ...
    {
        napi_wrappers::Value operator()(
            napi_wrappers::Runtime& runtime, const napi_wrappers::Value& thisVal, const napi_wrappers::Value* args, size_t count) const
        {
            if (count != data->arity)
            {
        ...
        projected_object_instance* instance;
    };
    ...
    struct projected_overloaded_function
    {
            ...
            napi_wrappers::Runtime& runtime, const napi_wrappers::Value& thisVal, const napi_wrappers::Value* args, size_t count) const
        {
            for (auto func : data)
            {
                if (func->arity == count)
                {
                    return func->function(runtime, instance->m_instance, args);
                }
            }
...
napi_wrappers::Value projected_object_instance::get(napi_wrappers::Runtime& runtime, const napi_wrappers::PropNameID& id)
{
    auto name = id.utf8(runtime);
    ...
    napi_wrappers::Value fallbackValue;
    for (auto iface : m_interfaces)
    {
...
void projected_object_instance::set(napi_wrappers::Runtime& runtime, const napi_wrappers::PropNameID& id, const napi_wrappers::Value& value)
{
    auto name = id.utf8(runtime);
    for (auto iface : m_interfaces)
    {
...
std::vector<napi_wrappers::PropNameID> projected_object_instance::getPropertyNames(napi_wrappers::Runtime& runtime)
{
...
napi_wrappers::Value projected_object_instance::add_event_listener(napi_wrappers::Runtime& runtime, const napi_wrappers::Value* args, size_t count)
{
    if (count < 2)
    {
        throw Napi::Error::New(runtime.env(), "TypeError: addEventListener expects (at least) 2 arguments");
...
napi_wrappers::Value projected_object_instance::remove_event_listener(napi_wrappers::Runtime& runtime, const napi_wrappers::Value* args, size_t count)
{
    if (count < 2)
    {
        throw Napi::Error::New(runtime.env(), "TypeError: removeEventListener expects (at least) 2 arguments");
...
bool projected_value_traits<bool>::as_native(napi_wrappers::Runtime&, const napi_wrappers::Value& value) noexcept
{
    if (value.isBool())
    {
...
napi_wrappers::Value projected_value_traits<char16_t>::as_value(napi_wrappers::Runtime& runtime, char16_t value)
{
    char buffer[8];
    ...
    winrt::check_bool(bytes);
    return napi_wrappers::String::createFromUtf8(runtime, reinterpret_cast<uint8_t*>(buffer), bytes);
}

char16_t projected_value_traits<char16_t>::as_native(napi_wrappers::Runtime& runtime, const napi_wrappers::Value& value)
{
    auto str = convert_value_to_native<winrt::hstring>(runtime, value);
    return str.empty() ? 0 : str[0];
}

winrt::hstring projected_value_traits<winrt::hstring>::as_native(napi_wrappers::Runtime& runtime, const napi_wrappers::Value& value)
{
...
napi_wrappers::Value projected_value_traits<winrt::guid>::as_value(napi_wrappers::Runtime& runtime, const winrt::guid& value)
{
    ...
    return napi_wrappers::Value(runtime, napi_wrappers::String::createFromAscii(runtime, buffer, uuid_length));
}

winrt::guid projected_value_traits<winrt::guid>::as_native(napi_wrappers::Runtime& runtime, const napi_wrappers::Value& value)
{
    auto str = value.asString(runtime).utf8(runtime);
    ...
    {
        throw Napi::Error::New(runtime.env(), "TypeError: Invalid GUID length");
    }
    ...
    if (::UuidFromStringA(reinterpret_cast<RPC_CSTR>(strBuffer), reinterpret_cast<UUID*>(winrt::put_abi(result))) !=
        ERROR_SUCCESS)
    {
        throw Napi::Error::New(runtime.env(), "TypeError: GUID contains unexpected characters");
    }
...
napi_wrappers::Value projected_value_traits<winrt::DateTime>::as_value(napi_wrappers::Runtime& runtime, winrt::DateTime value)
{
    auto unixTime =
...
winrt::DateTime projected_value_traits<winrt::DateTime>::as_native(napi_wrappers::Runtime& runtime, const napi_wrappers::Value& value)
{
    double number;
    if (value.isNumber())
    {
...
napi_wrappers::Value projected_value_traits<winrt::TimeSpan>::as_value(napi_wrappers::Runtime& runtime, winrt::TimeSpan value)
{
    auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(value);
    return convert_native_to_value<int64_t>(runtime, ms.count());
}

winrt::TimeSpan projected_value_traits<winrt::TimeSpan>::as_native(napi_wrappers::Runtime& runtime, const napi_wrappers::Value& value)
{
...
winrt::float3x2 projected_value_traits<winrt::float3x2>::as_native(napi_wrappers::Runtime& runtime, const napi_wrappers::Value& value)
{
...
napi_wrappers::Value projected_value_traits<winrt::float4x4>::as_value(napi_wrappers::Runtime& runtime, winrt::float4x4 value)
{
    napi_wrappers::Object result(runtime);
    result.setProperty(runtime, "m11", convert_native_to_value(runtime, value.m11));
    result.setProperty(runtime, "m12", convert_native_to_value(runtime, value.m12));
...
winrt::float4x4 projected_value_traits<winrt::float4x4>::as_native(napi_wrappers::Runtime& runtime, const napi_wrappers::Value& value)
{
...
napi_wrappers::Value projected_value_traits<winrt::plane>::as_value(napi_wrappers::Runtime& runtime, winrt::plane value)
{
    napi_wrappers::Object result(runtime);
...
winrt::plane projected_value_traits<winrt::plane>::as_native(napi_wrappers::Runtime& runtime, const napi_wrappers::Value& value)
{
    winrt::plane result{};
    auto obj = value.asObject(runtime);
...
napi_wrappers::Value projected_value_traits<winrt::quaternion>::as_value(napi_wrappers::Runtime& runtime, winrt::quaternion value)
{
    napi_wrappers::Object result(runtime);
...
winrt::quaternion projected_value_traits<winrt::quaternion>::as_native(napi_wrappers::Runtime& runtime, const napi_wrappers::Value& value)
{
...
napi_wrappers::Value projected_value_traits<winrt::float2>::as_value(napi_wrappers::Runtime& runtime, winrt::float2 value)
{
    napi_wrappers::Object result(runtime);
    result.setProperty(runtime, "x", convert_native_to_value(runtime, value.x));
    result.setProperty(runtime, "y", convert_native_to_value(runtime, value.y));
...
winrt::float2 projected_value_traits<winrt::float2>::as_native(napi_wrappers::Runtime& runtime, const napi_wrappers::Value& value)
{
    winrt::float2 result{};
    auto obj = value.asObject(runtime);
    if (auto field = obj.getProperty(runtime, "x"); !field.isUndefined())
...
napi_wrappers::Value projected_value_traits<winrt::float3>::as_value(napi_wrappers::Runtime& runtime, winrt::float3 value)
{
    napi_wrappers::Object result(runtime);
    result.setProperty(runtime, "x", convert_native_to_value(runtime, value.x));
    result.setProperty(runtime, "y", convert_native_to_value(runtime, value.y));
...
winrt::float3 projected_value_traits<winrt::float3>::as_native(napi_wrappers::Runtime& runtime, const napi_wrappers::Value& value)
{
    winrt::float3 result{};
    auto obj = value.asObject(runtime);
    if (auto field = obj.getProperty(runtime, "x"); !field.isUndefined())
...
napi_wrappers::Value projected_value_traits<winrt::float4>::as_value(napi_wrappers::Runtime& runtime, winrt::float4 value)
{
    napi_wrappers::Object result(runtime);
    result.setProperty(runtime, "x", convert_native_to_value(runtime, value.x));
    result.setProperty(runtime, "y", convert_native_to_value(runtime, value.y));
...
winrt::float4 projected_value_traits<winrt::float4>::as_native(napi_wrappers::Runtime& runtime, const napi_wrappers::Value& value)
{
    winrt::float4 result{};
    auto obj = value.asObject(runtime);
    if (auto field = obj.getProperty(runtime, "x"); !field.isUndefined())
...
napi_wrappers::Value rnwinrt::convert_from_property_value(napi_wrappers::Runtime& runtime, const winrt::IPropertyValue& value)
{
    switch (value.Type())
    {
    ...
    case winrt::PropertyType::Inspectable:
        return napi_wrappers::Value::undefined(runtime); // IInspectable is just the object itself
...
winrt::IInspectable rnwinrt::convert_to_property_value(napi_wrappers::Runtime& runtime, const napi_wrappers::Value& value)
{
    if (value.isBool())
    {
