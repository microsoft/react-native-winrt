...
public:
    ...
    CommandReader(const uint32_t argc, char** argv, const Option(&options)[NumOptions]) : CommandReader(argc, argv, &options[0], NumOptions)
    {
    }
    ...
    std::set<std::string> Files(const std::string_view& name, std::function<bool(std::string_view)> directoryFilter = nullptr) const;
