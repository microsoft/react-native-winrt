...
namespace rnwinrt::typeparser
{
    ...
    {
        return is_remove_overload(method) ||
               has_attribute(method, "Windows.Foundation.Metadata", "NoExceptionAttribute");
    }
    ...
    bool is_exclusive_to(TypeDef const& type)
    {
        return get_category(type) == category::interface_type &&
               has_attribute(type, "Windows.Foundation.Metadata"sv, "ExclusiveToAttribute"sv);
    }
    ...
    bool is_api_contract_type(TypeDef const& type)
    {
        return get_category(type) == category::struct_type &&
               has_attribute(type, "Windows.Foundation.Metadata"sv, "ApiContractAttribute"sv);
    }
    ...
    using type_semantics = std::variant<fundamental_type, object_type, guid_type, type_type, type_definition,
        generic_type_instance, generic_type_index, generic_type_param>;
    ...
    {
        return std::visit(
            impl::overloaded{ [](ElementType type) -> type_semantics {
                                 switch (type)
                                 {
                ...
                [](coded_index<TypeDefOrRef> type) -> type_semantics { return get_type_semantics(type); },
    ...
    inline const std::initializer_list<mapped_type> get_mapped_types_in_namespace(std::string_view typeNamespace)
    {
        static const struct
        {
            ...
            { "Microsoft.UI.Xaml.Interop",
                {
                    ...
                    { "NotifyCollectionChangedEventArgs", "System.Collections.Specialized",
                        "NotifyCollectionChangedEventArgs", true },
                    { "NotifyCollectionChangedEventHandler", "System.Collections.Specialized",
                        "NotifyCollectionChangedEventHandler", true },
                } },
            ...
            { "Microsoft.UI.Xaml.Media.Animation",
                { { "IKeyTimeHelper" }, { "IKeyTimeHelperStatics" }, { "IRepeatBehaviorHelper" },
                    { "IRepeatBehaviorHelperStatics" }, { "KeyTime", "Microsoft.UI.Xaml.Media.Animation", "KeyTime" },
                    { "KeyTimeHelper" }, { "RepeatBehavior", "Microsoft.UI.Xaml.Media.Animation", "RepeatBehavior" },
            ...
            { "Windows.Foundation",
                { { "DateTime", "System", "DateTimeOffset", true },
                    ...
                    { "HResult", "System", "Exception", true }, { "IClosable", "System", "IDisposable", true, true },
                    { "IPropertyValue", "Windows.Foundation", "IPropertyValue", true },
                    { "IReferenceArray`1", "Windows.Foundation", "IReferenceArray", true },
                    { "IReference`1", "System", "Nullable", true }, { "Point", "Windows.Foundation", "Point" },
                    { "Rect", "Windows.Foundation", "Rect" }, { "Size", "Windows.Foundation", "Size" },
                    { "TimeSpan", "System", "TimeSpan", true }, { "Uri", "System", "Uri", true } } },
            ...
            { "Windows.UI.Xaml.Interop", { { "TypeKind", "Windows.UI.Xaml.Interop", "TypeKind", true },
                                             { "TypeName", "System", "Type", true } } },
        };

        auto nsItr = std::lower_bound(std::begin(mapped_types), std::end(mapped_types), typeNamespace,
            [](auto&& v, std::string_view ns) { return v.name_space < ns; });
    ...
    inline const mapped_type* get_mapped_type(std::string_view typeNamespace, std::string_view typeName)
    {
        ...
        auto nameItr = std::lower_bound(mapped_types.begin(), mapped_types.end(), typeName,
            [](auto&& v, std::string_view name) { return v.abi_name < name; });
    ...
    inline const std::string_view get_contract_platform(std::string_view contract_name, int32_t contract_version)
    {
        ...
        auto contractItr = std::lower_bound(std::begin(contract_mappings), std::end(contract_mappings), contract_name,
            [](auto&& c, std::string_view contract_name) { return c.contract_name < contract_name; });
        ...
        auto versionItr = std::lower_bound(std::begin(versions), std::end(versions), contract_version,
            [](auto&& v, int32_t contract_version) { return v.contract_version < contract_version; });
