...
namespace winrt::%
{
)"sv, namespaceFullName);

    // Forward declare the struct and delegate values because the winrt headers are not included in the generated header or the pch.
...
{)", namespaceFullName, c_codegenNamespace);
    ...
    writer.Write(R"(} // %
)"sv, c_codegenNamespace);

    return converters;
}

void TryWriteStructConverterDefinition(FileWriter& writer, const std::set<std::string>& structs, const winmd::reader::TypeDef& structDef,
    std::map<std::string, std::vector<winmd::reader::TypeSig>>& specializedInterfaceReferences)
{
    ...
    for (const auto& field : structDef.FieldList())
    {
        ...
        result.setProperty(runtime, "%", %(context, value.%));)",
            jsName, GetNativeToValueConverter(writer, field.Signature().Type(), &specializedInterfaceReferences), nativeName);
    }
...
)", jsName, nativeName, GetValueToNativeConverter(writer, field.Signature().Type(), false /*isInout*/, &specializedInterfaceReferences));
    }
...
void TryWriteDelegateConverterDefinition(const Settings& settings, FileWriter& writer, const std::set<std::string>& delegates, const winmd::reader::TypeDef& delegateDef,
    std::map<std::string, std::vector<winmd::reader::TypeSig>>& specializedInterfaceReferences)
{
    ...
    const auto methodDef = std::find_if(begin(methods), end(methods), [](auto&& method) { return method.Name() == "Invoke"; });
    ...
    if (returnType)
    {
        // Like for methods, we assume that return values for delegate are ByRef so we ignore 'getterReturnType.ByRef()'.
        functionOverloadCreateParams.push_back("&" + GetNativeToValueConverter(writer, returnType.Type(), &specializedInterfaceReferences));
    }
    ...
    for (uint32_t i = 0; i < paramCount; ++i)
    {
        ...
        if (param.Flags().Out())
        {
            if (settings.Verbose)
            {
                std::cout << "TODO: Skipping " << delegateFullName << " as inout-params and out-params are not implemented\n";
            }
            return;
        }

        functionOverloadCreateParams.push_back(writer.WriteTemp(R"(&%)", GetValueToNativeConverter(writer, paramType, false /*isInout*/, &specializedInterfaceReferences)));
        delegateLambdaParams.push_back(writer.WriteTemp(R"(const %& %)", paramType, param.Name()));
        delegateCallParams.push_back(writer.WriteTemp(R"(%(context, %))", GetNativeToValueConverter(writer, paramType, &specializedInterfaceReferences), param.Name()));
    }

    // CallSync is used instead of winrt::impl::make_agile_delegate as the latter would only work if JS is running in an ASTA while
    // it appears that RNW uses ChakraCore and V8 using MTA so marshaling back to an MTA would likely use a different thread than
    // that of JS. Hence, if necessary the call (including value conversions) will be dispatched on the JS thread while the original
    // thread blocks. If the thread is already correct then the call can be made directly.
    std::string functionCall;
    if (returnType)
    {
        const auto& returnTypeSig = returnType.Type();
        const auto returnTypeConverter = GetValueToNativeConverter(writer, returnTypeSig, false /*isInout*/, &specializedInterfaceReferences);
        ...
        functionCall = writer.WriteTemp(R"(
                ...
                return value;)"sv, returnTypeSig, isReturnTypeValueType ? "{}"sv : "{ nullptr }"sv, returnTypeConverter, Writer::BindList(","sv, delegateCallParams));
    }
    else
    {
        functionCall = writer.WriteTemp(R"(
                ...
                );)"sv, Writer::BindList(", "sv, delegateCallParams));
    }
...
)", delegateFullName, delegateFullName, delegateFullName, Writer::BindList(", "sv, functionOverloadCreateParams), delegateFullName,
    delegateFullName, delegateFullName, delegateFullName, Writer::BindList(", "sv, delegateLambdaParams), functionCall);
}

void WriteValueConverterDefintions(const Settings& settings, FileWriter& writer, const Namespace& node, const ValueConverters& valueConverters, std::map<std::string, std::vector<winmd::reader::TypeSig>>& specializedInterfaceReferences)
{
...
{)"sv, c_codegenNamespace);
    ...
    {
        TryWriteDelegateConverterDefinition(settings, writer, valueConverters.Delegates, delegateDef, specializedInterfaceReferences);
    }
...
std::optional<std::string> TryWriteEnum(const Settings& settings, FileWriter& writer, const std::string namespaceFullName, const winmd::reader::TypeDef& enumDef)
{
...
)"sv, Writer::ToLowerCamelCase(field.Name()), namespaceFullName, enumName, field.Name());
#endif
        }
    }
...
writer.Write(R"(        };
    ...
    return writer.WriteTemp(R"(ProjectedEnum::Create("%"sv, &::%::%::%_InitEnum))"sv,
        enumName, c_codegenNamespace, namespaceFullName, enumName);
}
...
std::vector<std::string> WriteProperties(const Settings& settings, FileWriter& writer, const std::string interfaceFullName, const std::map<std::string_view, PropertyInfo>& properties, std::map<std::string, std::vector<winmd::reader::TypeSig>>& specializedInterfaceReferences)
{
    ...
    for (const auto property : properties)
    {
        ...
        if (propertyInfo.SetterDef && IsMethodAllowed(settings, propertyInfo.SetterDef))
        {
            ...
            // Like for methods, we assume that return values for properties are ByRef so we ignore 'getterReturnType.ByRef()'.
            propertyCreates.push_back(writer.WriteTemp(R"(ProjectedProperty::Create<winrt::%>("%"sv, &winrt::%, &%, &winrt::%, &%))",
                interfaceFullName, Writer::ToLowerCamelCase(property.first), propertyFullNameWinRT, GetNativeToValueConverter(writer, getterReturnTypeSig, &specializedInterfaceReferences),
                propertyFullNameWinRT, GetValueToNativeConverter(writer, setterParamSigs.first->Type(), false /*isInout*/, &specializedInterfaceReferences)));
        }
        else
        {
            // Like for methods, we assume that return values for properties are ByRef so we ignore 'getterReturnType.ByRef()'.
...
std::optional<std::string> TryWriteMethodOverload(const Settings& settings, FileWriter& writer, const std::string interfaceFullName, const std::string_view& methodName, const MethodOverloadInfo& methodOverload,
    const bool useConstrainFunctionOverload, std::map<std::string, std::vector<winmd::reader::TypeSig>>& specializedInterfaceReferences)
{
    ...
    if (useConstrainFunctionOverload)
    {
        if (settings.Verbose)
        {
            std::cout << "TODO: Skipping an overload of " << interfaceFullName << "::" << methodName << " with arity " << methodOverload.InParamCount << " and " << methodOverload.OutParamCount << " out-params since code-gen for ConstrainFunctionOverload is not implemented yet\n";
        }
        return {};
    }
    ...
    if (returnType)
    {
        ...
        // Since this is a return value, if it is an array, we should ignore returnType.ByRef() which will be false and assume it is ByRef.
    ...
    for (uint32_t i = 0; i < paramCount; ++i)
    {
        ...
        if (hasNoOutParams)
        {
            createParams.push_back(writer.WriteTemp(R"(&%)", GetValueToNativeConverter(writer, paramType, isInout, &specializedInterfaceReferences)));
        }
        else if (param.Flags().In() || isInout)
        {
            createParams.push_back(writer.WriteTemp(R"(ProjectedFunctionParamInfo(ProjectedFunctionInParam, &%))"sv, GetValueToNativeConverter(writer, paramType, isInout, &specializedInterfaceReferences)));
        }
        else
        {
            ...
            createParams.push_back(writer.WriteTemp(R"(ProjectedFunctionParamInfo(ProjectedFunctionOutParam, &%, "%"))"sv, GetNativeToValueConverter(writer, paramType, &specializedInterfaceReferences), Writer::ToLowerCamelCase(param.Name())));
        }
    }

    return writer.WriteTemp(R"(ProjectedFunctionOverload::Create<winrt::%>(%))", interfaceFullName, Writer::BindList(", "sv, createParams));
}

std::optional<std::string> TryWriteMethod(const Settings& settings, FileWriter& writer, const std::string_view interfaceName, const std::string interfaceFullName, const std::string_view& methodName,
    const std::map<uint32_t, std::vector<MethodOverloadInfo>>& methodOverloads, std::map<std::string, std::vector<winmd::reader::TypeSig>>& specializedInterfaceReferences)
{
    ...
    for (const auto& methodOverloadArities : methodOverloads)
    {
        ...
        // JS cannot overload by types, only by the number of in-params so take the first method with that number of params found
        // and don't codegen for the other(s). Note that if there are overloads with the same arity on different interfaces,
        // it will be resolved by picking the first it finds at runtime. In the public SDK, probematic overloads include:
        ...
        if (settings.Verbose)
        {
            for (uint32_t i = 0; i < methodOverloadArities.second.size(); ++i)
            {
                if (i != defaultIndex)
                {
                    std::cout << "INFO: Skipping an ambiguous overload of " << interfaceFullName << "::" << methodName << " with arity " << methodOverloadArities.first << " and " << methodOverloadArities.second[i].OutParamCount << " out-params\n";
                }
            }
        }

        const auto& methodOverload = methodOverloadArities.second[defaultIndex];
        if (auto projectedMethodOverloadCreate = TryWriteMethodOverload(settings, writer, interfaceFullName, methodName, methodOverloadArities.second[defaultIndex],
                !isTotalParamCountUnique[methodOverload.InParamCount + methodOverload.OutParamCount], specializedInterfaceReferences))
        {
    ...
    {
        return writer.WriteTemp(R"(ProjectedFunction::Create("%"sv, %))", Writer::ToLowerCamelCase(methodName), projectedMethodOverloadCreates[0]);
    }
    ...
    return writer.WriteTemp(R"(ProjectedFunction::Create("%"sv, &::%::%_%_InitMethod))", Writer::ToLowerCamelCase(methodName), c_codegenNamespace, interfaceFullName, methodName);
}

std::vector<std::string> WriteMethods(const Settings& settings, FileWriter& writer, const std::string_view interfaceName, const std::string interfaceFullName,
    const std::map<std::string_view, std::map<uint32_t, std::vector<MethodOverloadInfo>>>& methods,std::map<std::string, std::vector<winmd::reader::TypeSig>>& specializedInterfaceReferences)
{
    ...
    for (const auto& method : methods)
    {
        ...
        if (auto projectedMethodCreate = TryWriteMethod(settings, writer, interfaceName, interfaceFullName, method.first, method.second, specializedInterfaceReferences))
        {
...
std::vector<std::string> WriteEvents(const Settings& settings, FileWriter& writer, const std::string_view interfaceName, const std::string interfaceFullName,
    const std::map<std::string_view, EventInfo>& events, std::map<std::string, std::vector<winmd::reader::TypeSig>>& specializedInterfaceReferences)
{
    ...
    for (const auto event : events)
    {
        ...
        if (!eventInfo.AddDef || !eventInfo.RemoveDef || !IsMethodAllowed(settings, eventInfo.AddDef) || !IsMethodAllowed(settings, eventInfo.RemoveDef))
        {
            continue;
        }
        ...
        eventCreates.push_back(writer.WriteTemp(R"(ProjectedEvent::Create<winrt::%>("%"sv, &winrt::%, &winrt::%, &%))"sv, interfaceFullName, Writer::ToLowerCase(event.first), eventFullName, eventFullName,
...
std::optional<std::string> TryWriteInterface(const Settings& settings, FileWriter& writer, const std::string& namespaceFullName, const winmd::reader::TypeDef& interfaceDef, std::map<std::string, std::vector<winmd::reader::TypeSig>>& specializedInterfaceReferences)
{
    ...
    // Omit the Windows.Foundation generic-param types like IAsyncOperation<T> and IVectorView<T> as we need to include the specialized
    // versions in the interface tables in order to be usable since they each have a unique IID per specialization.
    if (!winmd::reader::empty(interfaceDef.GenericParam()))
    {
        return {};
    }

    const auto projectedPropertyCreates = WriteProperties(settings, writer, interfaceFullName, interfaceInfo.Properties, specializedInterfaceReferences);
    const auto projectedMethodCreates = WriteMethods(settings, writer, interfaceName, interfaceFullName, interfaceInfo.Methods, specializedInterfaceReferences);
    const auto projectedEventCreates = WriteEvents(settings, writer, interfaceName, interfaceFullName, interfaceInfo.Events, specializedInterfaceReferences);
...
writer.Write(R"(            }
    ...
    return writer.WriteTemp(R"(ProjectedInterface::Create<winrt::%::%>(&::%::%::%_InitInterface))",
        namespaceFullName, interfaceName, c_codegenNamespace, namespaceFullName, interfaceName);
}

std::vector<std::string> WriteInterfaces(const Settings& settings, FileWriter& writer, const Namespace& node, std::map<std::string, std::vector<winmd::reader::TypeSig>>& specializedInterfaceReferences)
{
    ...
    {
        if (auto projectedInterfaceCreate = TryWriteInterface(settings, writer, namespaceFullName, interfaceDef, specializedInterfaceReferences))
        {
...
void SpecializeIterator(FileWriter& writer, const winmd::reader::TypeSig& valueTypeSig, std::set<std::string>& projectedInterfaceCreates)
{
    projectedInterfaceCreates.insert(writer.WriteTemp("ProjectedInterface::Create<winrt::Windows::Foundation::Collections::IIterator<%>>(&::WinRTTurboModule::Windows::Foundation::Collections::InitIIteratorInterface<%, %>)"sv,
        valueTypeSig, valueTypeSig, GetNativeToValueConverter(writer, valueTypeSig, nullptr /*specializedInterfaceReferences*/)));
}

void SpecializeIterable(FileWriter& writer, const winmd::reader::TypeSig& valueTypeSig, std::set<std::string>& projectedInterfaceCreates)
{
    projectedInterfaceCreates.insert(writer.WriteTemp("ProjectedInterface::Create<winrt::Windows::Foundation::Collections::IIterable<%>>(&::WinRTTurboModule::Windows::Foundation::Collections::InitIIterableInterface<%>)"sv,
        valueTypeSig, valueTypeSig));
    SpecializeIterator(writer, valueTypeSig, projectedInterfaceCreates);
}

void SpecializeVectorView(FileWriter& writer, const winmd::reader::TypeSig& valueTypeSig, std::set<std::string>& projectedInterfaceCreates)
{
    projectedInterfaceCreates.insert(writer.WriteTemp("ProjectedInterface::Create<winrt::Windows::Foundation::Collections::IVectorView<%>>(&::WinRTTurboModule::Windows::Foundation::Collections::InitIVectorViewInterface<%, %, %>)"sv,
        valueTypeSig, valueTypeSig, GetNativeToValueConverter(writer, valueTypeSig, nullptr /*specializedInterfaceReferences*/), GetValueToNativeConverter(writer, valueTypeSig, false /*isInout*/, nullptr /*specializedInterfaceReferences*/)));
    SpecializeIterable(writer, valueTypeSig, projectedInterfaceCreates);
}

void SpecializeVector(FileWriter& writer, const winmd::reader::TypeSig& valueTypeSig, std::set<std::string>& projectedInterfaceCreates)
{
    projectedInterfaceCreates.insert(writer.WriteTemp("ProjectedInterface::Create<winrt::Windows::Foundation::Collections::IVector<%>>(&::WinRTTurboModule::Windows::Foundation::Collections::InitIVectorInterface<%, %, %>)"sv,
        valueTypeSig, valueTypeSig, GetNativeToValueConverter(writer, valueTypeSig, nullptr /*specializedInterfaceReferences*/), GetValueToNativeConverter(writer, valueTypeSig, false /*isInout*/, nullptr /*specializedInterfaceReferences*/)));
    SpecializeVectorView(writer, valueTypeSig, projectedInterfaceCreates);
    SpecializeIterable(writer, valueTypeSig, projectedInterfaceCreates);
}

void SpecializeObservableVector(FileWriter& writer, const winmd::reader::TypeSig& valueTypeSig, std::set<std::string>& projectedInterfaceCreates)
{
    projectedInterfaceCreates.insert(writer.WriteTemp("ProjectedInterface::Create<winrt::Windows::Foundation::Collections::IObservableVector<%>>(&::WinRTTurboModule::Windows::Foundation::Collections::InitIObservableVectorInterface<%>)"sv,
        valueTypeSig, valueTypeSig));
    SpecializeVector(writer, valueTypeSig, projectedInterfaceCreates);
}

void SpecializeKeyValuePair(FileWriter& writer, const winmd::reader::TypeSig& keyTypeSig, const winmd::reader::TypeSig& valueTypeSig, std::set<std::string>& projectedInterfaceCreates)
{
    projectedInterfaceCreates.insert(writer.WriteTemp("ProjectedInterface::Create<winrt::Windows::Foundation::Collections::IKeyValuePair<%, %>>(&::WinRTTurboModule::Windows::Foundation::Collections::InitIKeyValuePairInterface<%, %, %, %>)"sv,
        keyTypeSig, valueTypeSig, keyTypeSig, valueTypeSig, GetNativeToValueConverter(writer, keyTypeSig, nullptr /*specializedInterfaceReferences*/), GetNativeToValueConverter(writer, valueTypeSig, nullptr /*specializedInterfaceReferences*/)));
}

void SpecializeKeyValuePairIterator(FileWriter& writer, const winmd::reader::TypeSig& keyTypeSig, const winmd::reader::TypeSig& valueTypeSig, std::set<std::string>& projectedInterfaceCreates)
{
    const auto keyValuePairInterface = writer.WriteTemp("winrt::Windows::Foundation::Collections::IKeyValuePair<%, %>"sv, keyTypeSig, valueTypeSig);
    projectedInterfaceCreates.insert(writer.WriteTemp("ProjectedInterface::Create<winrt::Windows::Foundation::Collections::IIterator<%>>(&::WinRTTurboModule::Windows::Foundation::Collections::InitIIteratorInterface<%, ConvertInterfaceToValue<%>>)"sv,
...
void SpecializeKeyValuePairIterable(FileWriter& writer, const winmd::reader::TypeSig& keyTypeSig, const winmd::reader::TypeSig& valueTypeSig, std::set<std::string>& projectedInterfaceCreates)
{
    const auto keyValuePairInterface = writer.WriteTemp("winrt::Windows::Foundation::Collections::IKeyValuePair<%, %>"sv, keyTypeSig, valueTypeSig);
    projectedInterfaceCreates.insert(writer.WriteTemp("ProjectedInterface::Create<winrt::Windows::Foundation::Collections::IIterable<%>>(&::WinRTTurboModule::Windows::Foundation::Collections::InitIIterableInterface<%>)"sv,
...
void SpecializeMap(FileWriter& writer, const winmd::reader::TypeSig& keyTypeSig, const winmd::reader::TypeSig& valueTypeSig, std::set<std::string>& projectedInterfaceCreates)
{
    projectedInterfaceCreates.insert(writer.WriteTemp("ProjectedInterface::Create<winrt::Windows::Foundation::Collections::IMap<%, %>>(&::WinRTTurboModule::Windows::Foundation::Collections::InitIMapInterface<%, %, %, %, %>)"sv,
        keyTypeSig, valueTypeSig, keyTypeSig, valueTypeSig, GetValueToNativeConverter(writer, keyTypeSig, false /*isInout*/, nullptr /*specializedInterfaceReferences*/), GetNativeToValueConverter(writer, valueTypeSig, nullptr /*specializedInterfaceReferences*/),
        GetValueToNativeConverter(writer, valueTypeSig, false /*isInout*/, nullptr /*specializedInterfaceReferences*/)));
    SpecializeKeyValuePairIterable(writer, keyTypeSig, valueTypeSig, projectedInterfaceCreates);
}

void SpecializeMapView(FileWriter& writer, const winmd::reader::TypeSig& keyTypeSig, const winmd::reader::TypeSig& valueTypeSig, std::set<std::string>& projectedInterfaceCreates)
{
    projectedInterfaceCreates.insert(writer.WriteTemp("ProjectedInterface::Create<winrt::Windows::Foundation::Collections::IMapView<%, %>>(&::WinRTTurboModule::Windows::Foundation::Collections::InitIMapViewInterface<%, %, %, %>)"sv,
        keyTypeSig, valueTypeSig, keyTypeSig, valueTypeSig, GetValueToNativeConverter(writer, keyTypeSig,  false /*isInout*/, nullptr /*specializedInterfaceReferences*/), GetNativeToValueConverter(writer, valueTypeSig, nullptr /*specializedInterfaceReferences*/)));
    SpecializeKeyValuePairIterable(writer, keyTypeSig, valueTypeSig, projectedInterfaceCreates);
}

void SpecializeMapChangedEventArgs(FileWriter& writer, const winmd::reader::TypeSig& keyTypeSig, std::set<std::string>& projectedInterfaceCreates)
{
    projectedInterfaceCreates.insert(writer.WriteTemp("ProjectedInterface::Create<winrt::Windows::Foundation::Collections::IMapChangedEventArgs<%>>(&::WinRTTurboModule::Windows::Foundation::Collections::InitIMapChangedEventArgsInterface<%, %>)"sv,
        keyTypeSig, keyTypeSig, GetNativeToValueConverter(writer, keyTypeSig, nullptr /*specializedInterfaceReferences*/)));
}

void SpecializeObservableMap(FileWriter& writer, const winmd::reader::TypeSig& keyTypeSig, const winmd::reader::TypeSig& valueTypeSig, std::set<std::string>& projectedInterfaceCreates)
{
    projectedInterfaceCreates.insert(writer.WriteTemp("ProjectedInterface::Create<winrt::Windows::Foundation::Collections::IObservableMap<%, %>>(&::WinRTTurboModule::Windows::Foundation::Collections::InitIObservableMapInterface<%, %>)"sv,
...
std::set<std::string> SpecializeInterfaces(FileWriter& writer, const std::map<std::string, std::vector<winmd::reader::TypeSig>>& specializedInterfaceReferences)
{
    ...
    // PERF: It would likely be more performant to remove redundant types (e.g. IIterable<T> if we find a IVector<T>) before specializing.
    ...
    for (const auto& reference : specializedInterfaceReferences)
    {
        ...
        // IAsyncActionWithProgress, IAsyncOperation, IAsyncOperationWithProgress are not specialized since we already generate a wrapper
        // to make them act like special promises so there wouldn't be any more functionality made available by doing so. We generate
        // converters for IReference and IReferenceArray so they also do not need to be specialized.
    }
...
std::optional<std::string> TryWriteRuntimeClass(const Settings& settings, FileWriter& writer, const std::string& namespaceFullName, const winmd::reader::TypeDef& classDef,
    std::map<std::string, std::vector<winmd::reader::TypeSig>>& specializedInterfaceReferences)
{
    ...
    // Because there is no code-generation for the runtime class (instance) itself and only interfaces, we need to make sure
    // that if classes implement generic interfaces that the specialized implementations are included in the interface table.
    // While the 'specializedInterfaceReferences' otherwise accumulates specialized interfaces accumulated when generating
    // value converters, if a specialized interface is implemented by a runtime class and not otherwise referenced by non
    // generic interfaces, it might not otherwise found. e.g IMapView<winrt::hstring, Windows::ApplicationModel::Resources::Core::NamedResource>
    for (auto& interfaceImpl : classDef.InterfaceImpl())
    {
        ...
        if (type.type() == winmd::reader::TypeDefOrRef::TypeSpec)
        {
            ...
            if (specializedInterfaceReferences.find(specializedInterfaceName) == specializedInterfaceReferences.end())
            {
                ...
                specializedInterfaceReferences.emplace(std::move(specializedInterfaceName), std::move(genericParamTypeSigs));
            }
        }
    }
    ...
    { 
        if (factoryInterface.IsActivatable)
        {
            ...
            {
                activationFactoryInterfaceIds.push_back(writer.WriteTemp(R"(winrt::guid_of<%>())", factoryInterface.TypeDef));
            }
            else
            {
...
std::vector<std::string> WriteRuntimeClasses(const Settings& settings, FileWriter& writer, const Namespace& node, std::map<std::string, std::vector<winmd::reader::TypeSig>>& specializedInterfaceReferences)
{
    ...
    {
        if (auto projectedActivationFactoryCreate = TryWriteRuntimeClass(settings, writer, namespaceFullName, classDef, specializedInterfaceReferences))
        {
...
void WriteNamespace(const Settings& settings, FileWriter& writer, const Namespace& node, const ValueConverters& valueConverters,
    std::set<std::string>& specializedInterfaceCreates, uint32_t& numInterfaces, bool& includeCppWinRtHeader)
{
    numInterfaces = 0;
    ...
    writer.Write(R"(
namespace %::%
{)"sv, c_codegenNamespace, namespaceFullName);
    ...
    {
        // We generate the create statements for all the interfaces referenced in this namespace and then determine which have already
        // been included by a different namespaces. The intent is to collocate specialized interfaces with their use and to avoid collecting
        // them all and generating one huge GetInterfaces() in Windows.Foundation.Collections.g.cpp as that would create a massive object
        // file and have slow compilation speeds (single-threaded compilation per translation unit) while using a lot of memory.
        auto localSpecializedInterfaceCreates = SpecializeInterfaces(writer, specializedInterfaceReferences);
        projectedInterfaceCreates.resize(specializedInterfaceCreateStartIndex + localSpecializedInterfaceCreates.size());
        projectedInterfaceCreates.resize(std::distance(projectedInterfaceCreates.begin(), std::set_difference(
            std::make_move_iterator(localSpecializedInterfaceCreates.begin()), std::make_move_iterator(localSpecializedInterfaceCreates.end()),
            specializedInterfaceCreates.cbegin(), specializedInterfaceCreates.cend(), projectedInterfaceCreates.begin() + specializedInterfaceCreateStartIndex)));
    }

    includeCppWinRtHeader = valueConverters || !projectedEnumCreates.empty() || !projectedInterfaceCreates.empty() || !projectedFactoryCreates.empty();
    ...
    {
        writer.Write(R"(            %,
)"sv,  projectedEnumCreate);
    }

    for (const auto& projectedFactoryCreate : projectedFactoryCreates)
    {
        writer.Write(R"(            %,
)"sv,  projectedFactoryCreate);
    }

    // Namespaces are enumerated in post-order so the child namespace is already defined. It is guaranteed to be emitted even if it ends up empty.
    for (const auto& child : node.Children())
    {
        const auto& childNode = *child.second;
        writer.Write(R"(            ProjectedNamespace::Create("%"sv, context, &::%::%::InitNamespace),
)"sv, childNode.Name(), c_codegenNamespace, childNode.FullName(true /*useCppDelim*/));
    }
...
writer.Write(R"(        };
    ...
    {
         projectedInterfaceCreates.insert(projectedInterfaceCreates.end(), std::make_move_iterator(projectedInterfaceCreates.begin() + specializedInterfaceCreateStartIndex), std::make_move_iterator(projectedInterfaceCreates.end()));
    }
...
void WriteProjectionsGeneratedH(const Settings& settings, const std::map<std::string_view, std::shared_ptr<Namespace>>& roots)
{
...
)"sv, c_codegenNamespace, c_codegenNamespace);
}

std::map<std::string, ValueConverters> WriteProjectedValueConvertersGeneratedH(const Settings& settings, const std::map<std::string_view, std::shared_ptr<Namespace>>& roots)
{
    ...
    {
        valueConverters.emplace(node.FullName(true /*usCppDelim*/), WriteValueConverterDeclarations(settings, writer, node));
        return true;
    });
    return valueConverters;
}

void WriteNamespaceGenerateCppH(const Settings& settings, const Namespace& node, const std::map<std::string, ValueConverters> valueConverters,
    std::set<std::string>& specializedInterfaceCreates, std::map<std::string, uint32_t>& namespaceInterfaceCounts)
{
    const auto namespaceFullName = node.FullName(true /*useCppDelim*/);
...
void WriteNamespacesGenerateCppH(const Settings& settings, const std::map<std::string_view, std::shared_ptr<Namespace>>& roots, const std::map<std::string, ValueConverters> valueConverters, std::map<std::string, uint32_t>& namespaceInterfaceCounts)
{
    std::set<std::string> specializedInterfaceCreates;
    Namespace::EnumerateAll(roots, [&](const Namespace& node)
    {
        WriteNamespaceGenerateCppH(settings, node, valueConverters, specializedInterfaceCreates, namespaceInterfaceCounts);
        return true;
    });
}

void WriteProjectionsGeneratedCpp(const Settings& settings, const std::map<std::string_view, std::shared_ptr<Namespace>>& roots, const std::map<std::string, uint32_t>& namespaceInterfaceCounts)
{
    ...
    // Write includes for all generated namespaces since we need access to namespace and interface initializer functions.
    Namespace::EnumerateAll(roots, [&](const Namespace& node)
    {
...
namespace %
{
    ...
    {
        writer.Write(R"(            ProjectedNamespace::Create("%"sv, context, &::%::%::InitNamespace),
)"sv, root.first, c_codegenNamespace, root.first);
    }
