        ...
        tsWriter.Write("declare namespace % {\n%\n}"sv, node.FullName(), [&]() -> void
            {
                for (auto&& [name, type] : node.Members().types)
                {
                    ...
                    case winmd::reader::category::class_type:
                        if (true)
                        {
                            auto [namespc, name] = winmd::reader::get_base_class_namespace_and_name(type);
                            if (name != "Object")
                            {
                                //std::cout << type.TypeNamespace() << "." << type.TypeName() << ": " << namespc << "." << name << std::endl; 
                            }
                        }
                        else if (type.TypeName() == "ResourceCandidateVectorView")
                        {
                            /*std::cout << bool(type.Extends()) << std::endl;
                            if (type.Extends())
                            {
                                std::cout << type.Extends().TypeDef().TypeName() << std::endl;
                            }*/
                            auto [namespc, name] = winmd::reader::get_base_class_namespace_and_name(type);
                            std::cout << namespc << name << std::endl;
/*                            if (type.Extends().type() == winmd::reader::TypeDefOrRef::TypeDef)
                            {
                                auto const def = type.Extends().TypeDef();
                                std::cout << def.TypeName();
                            }
                            else if (type.Extends().type() == winmd::reader::TypeDefOrRef::TypeRef)
                            {
                                auto const ref = type.Extends().TypeRef();
                                std::cout << ref.TypeName();
                                auto x = type.CustomAttribute();
                            }
                            else
                            {
                                XLANG_ASSERT(false);
                        ...
                        else if (type.TypeName() == "ResourceContext")
                        {
                            std::cout << bool(type.Extends()) << std::endl;
                            for (auto& type: type.Extends().get_database().TypeDef)
                            {
                        ...
                        if (true)
                        {
                            //std::cout << node.FullName() << std::endl;
                            //std::cout << type.TypeName() << std::endl;
                            /*std::cout << "Abstract: " << type.Flags().Abstract() << std::endl;
                            std::cout << "Visibility: " << (type.Flags().Visibility() == winmd::reader::TypeVisibility::Public) << std::endl;
                            std::cout << "Sealed: " << type.Flags().Sealed() <<std::endl;
                            std::cout << "Extends: " << type.Extends().type().TypeName() << std::endl;*/
                            for (auto&& prop : type.PropertyList())
                            {
                                std::cout << "PROP " << prop.Name() ; 
                                //std::cout << " " << static_cast<UINT>(prop.Type().Type().element_type()) << std::endl;
                                if (prop.Type().Type().element_type() == winmd::reader::ElementType::ValueType)
                                {
                                    auto propertyTypeCI =
                                        std::get<winmd::reader::coded_index<winmd::reader::TypeDefOrRef>>(prop.Type().Type().Type());
                                    if (propertyTypeCI.type() == winmd::reader::TypeDefOrRef::TypeDef)
                                    {
                                        std::cout << ": " << propertyTypeCI.TypeDef().TypeName();
                                    }
                                    else if (propertyTypeCI.type() == winmd::reader::TypeDefOrRef::TypeRef)
                                    {
                                        std::cout << ": " << propertyTypeCI.TypeRef().TypeName();
                                    }                                    
                                }
                                std::cout << std::endl;
                            }
                            ...
                            for (auto&& method : type.MethodList())
                            {
                                ...
                                jswinrt::typeparser::method_signature sig(method);
                                for (auto&& [param, paramSig] : sig.params())
                                {
                                    std::cout << param.Name() << ": ";
                                    //auto&& [namespac, name] = TypescriptWriter::ParseType(paramSig->Type());
                                    //std::cout << namespac << "." << name << ", ";
                                    jswinrt::typeparser::type_semantics h = jswinrt::typeparser::get_type_semantics(paramSig->Type());
                                    if (std::holds_alternative<jswinrt::typeparser::type_definition>(h))
                                    {
                                        auto typeDef = std::get<jswinrt::typeparser::type_definition>(h);
                                        std::cout << typeDef.TypeNamespace() << "." << type.TypeName() << std::endl;
                                    }
                                    else if (std::holds_alternative<jswinrt::typeparser::fundamental_type>(h))
                                    {
                                        auto typeDef = std::get<jswinrt::typeparser::fundamental_type>(h);
                                        if (typeDef == jswinrt::typeparser::fundamental_type::String)
                                        {
                                            std::cout << "string" << std::endl;
                                        }
                                    }
                                    else if (std::holds_alternative<jswinrt::typeparser::generic_type_instance>(h))
                                    {
                                        auto typeDef = std::get<jswinrt::typeparser::generic_type_instance>(h);
                                        std::cout << typeDef.generic_type.TypeName() << "<>";
                                    }
                                }
                                ...
                                auto x = method.Signature().Params();
                                for (auto paramType : method.Signature().Params() )
                                {
                                    auto&& [namespac, name] = TypescriptWriter::ParseType(paramType.Type());
                                    std::cout << namespac << "." << name << ", " << std::endl;
                                }*/
                                std::cout << ")" << std::endl;
                            }
                        }
                    ...
                    case winmd::reader::category::enum_type:
                        ...
                        {
                            tsWriter.AddIndent();
                            tsWriter.WriteIndentedLine("enum % {%}"sv, type.TypeName(), [&]() -> void 
                                {
                                    tsWriter.AddIndent();
                                    for (auto field : typeN.FieldList())
                                    {
                                        tsWriter.WriteIndentedLine("%,"sv, field.Name());
                                        //std::cout << "-" << field.Name() << std::endl;
                                    }
                                    tsWriter.ReduceIndent();
                                }
                            );
                            tsWriter.ReduceIndent();
                            //std::cout << type.TypeName() << std::endl;
